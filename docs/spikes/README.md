...
The local-AI spikes went with `src/gen/` — `gen-clip.ts`, `clip_rank.py`, `flags/` and `vision/`.
Nothing left in the repo generates a model, so a spike that measures a generator has nothing to
measure. They are in git.
