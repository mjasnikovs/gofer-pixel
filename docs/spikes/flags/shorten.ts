...
const OUT = 'out/short-run'
const CANVAS = 32
const RENDER = 200
const SEEDS = [4200, 4201, 4202]
const SUBJECTS = ['a cat', 'a fox', 'a chicken', 'a knight', 'a red mushroom', 'a stone tower'] as const
const ARMS = ['long', 'short'] as const
...
const TEACHER: Readonly<Record<string, string>> = {
    ...
    'a chicken': 'chicken',
    'a knight': 'farmer',
    'a red mushroom': 'mushroom',
    'a stone tower': 'tower'
}

const SHORT: Record<string, string> = {}
for (const id of ['dog', 'chicken', 'farmer', 'mushroom', 'tower']) {
    SHORT[id] = await readFile(`out/short/${id}.js`, 'utf8')
}

const replyFor = (prompt: string, arm: Arm): string => {
    const id = TEACHER[prompt] ?? 'dog'
    return arm === 'long' ? (BUILT_IN_REPLIES[id] ?? '') : (SHORT[id] ?? '')
}

for (const id of ['dog', 'chicken', 'farmer', 'mushroom', 'tower']) {
    process.stdout.write(
        `${id.padEnd(10)}long ${String((BUILT_IN_REPLIES[id] ?? '').split('\n').length).padStart(3)}  ->  short ${String((SHORT[id] ?? '').trim().split('\n').length).padStart(3)}\n`
    )
}
            ...
            const response = await fetch(`${ENDPOINT}/v1/chat/completions`, {
                ...
                body: JSON.stringify({
                    messages: [
                        ...
                        {role: 'user', content: `a ${TEACHER[job.prompt] ?? 'dog'}`},
                        {role: 'assistant', content: replyFor(job.prompt, job.arm)},
