...
## Local AI — carried back 2026-08-08, removed 2026-08-17

It was never one of the seventeen, and it is not in `FEATURESET.md` or either mockup. It was carried
out of `legacy/` and rebuilt against the raycaster: the op language, the worked-example bank, the
deterministic scorers, six experiment flags and a dialog behind the main menu. It is gone now, whole
— `src/gen/`, `src/assets/examples/`, `browser/generate.spec.ts`, `docs/GEN_*.md`, `docs/AIGEN-*.md`
and the flag and vision spikes.

What is left is a disabled row in the main menu and an empty `GenerateDialog.tsx`. The `.gpix`
`origin` field went with it: a version-3 file still opens whole, and its provenance line is read
past, because nothing in the app can produce one or show one.

Everything that was measured — that the model cannot draw pixels, that it cannot rank its own
renders, that one worked example beats every rule in prose, that three seeds can only see a
threefold effect — is in git history and nowhere else. Read it before rebuilding any of this.
